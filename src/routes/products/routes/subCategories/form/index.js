...
import { Form, Input, Tooltip, Icon, Button, Spin, Select } from 'antd';
...
class FormComponent extends React.Component {
  ...
  editApi = () => {
    ...
    axios
      .patch(
        `${backendUrl}/edit-subcategory/${_id}`,
        {
          title,
          description,
          category_id,
  ...
  render() {
    ...
    const { title, description, category_id, categories, isLoading, apiError, error } = this.state;
    const { errorType, errorText } = error;
    if (type === 'edit') {
      if (!editData) {
        history.push(
